...
class NmRename(object):
	...
	def _rename(self, renamer, pathlist):
		...
		for old, new in pathlist:
			newfile = renamer.rename(new[1] if not self._renamedirs else os.path.join(*new), os.path.join(*old))
			if not newfile:
				print 'Ommitting %s. Would result in empty filename.' % new[1]
				newfile = new[1]
			newpathlist.append((old, (new[0], newfile) if not self._renamedirs else (os.path.dirname(newfile), os.path.basename(newfile))))
		return newpathlist
	...
	def _print_help():
		...
		for start, text  in helptexts:
			print '%s : %s' % (start+' '*(maxlen-len(start)), text.strip().replace('\n', '\n'+' '*(maxlen+3)))
...
'''
		...
		def rename(self, old, realold):
			...
			if not tagdata:
				...
				return self.withrenamedict(self.pattern, {
					'''%a''': tagdata['artist'][0] if 'artist' in tagdata else 'Unkown',
					'''%A''': tagdata['album'][0] if 'album' in tagdata else 'Unkown',
					'''%AA''': tagdata['albumartist'][0] if 'albumartist' in tagdata else tagdata['artist'][0] if 'artist' in tagdata else 'Unkown',
					'''%t''': tagdata['title'][0] if 'title' in tagdata else 'Unkown',
					'''%y''': tagdata['date'][0] if 'date' in tagdata else '0000',
					'''%tn''': tagdata['tracknumber'][0] if 'tracknumber' in tagdata else '0',
					'''%TN''': tagdata['tracktotal'][0] if 'tracktotal' in tagdata else '0',
					'''%e''': ext,
					})
except:
